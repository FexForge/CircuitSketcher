...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        QShortcut(QKeySequence.Cancel, self, activated=self.activate_select_mode)
    ...
    def activate_select_mode(self):
        ...
        Bricht zusätzlich eine eventuell laufende Leitungs-Zeichnung ab,
        weil on_component_selected('select') diesen Zustand nicht zurücksetzt.
        """
    ...
    def canvas_mouse_press(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            # Rechtsklick beendet das Leitungszeichnen (wie ESC): laufendes
            # Segment verwerfen, fertige Segmente bleiben, Auswahlmodus.
            if self.drawing_wire and self.temp_wire:
                self.activate_select_mode()
                self.statusBar().showMessage(
                    "Leitungszeichnen beendet – fertige Segmente bleiben."
                )
                event.accept()
                return
    ...
    def canvas_mouse_move(self, event):
        ...
        if self.drawing_wire and self.temp_wire:
            ...
            self.temp_wire.update_end_point(snapped_pos)
              ...
              and self.selected_component_type not in ["wire", "select", "voltage", "current"]):
    ...
    def _update_window_title(self):
        ...
        self.setWindowTitle(f"SchaltungsZeichner - {name}{star}")
